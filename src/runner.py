...
def run_cell(model, tokenizer, seq_len, cache_factory=None, n_runs=10, n_warmup=3):
    ...
    kv_cache_size_observed, kv_cache_size_theoretical = None, None
    if cache_factory is None:
        with torch.inference_mode():
            output = model(input_ids, use_cache=True, logits_to_keep=1)
        
        cache = output.past_key_values
        # Observed and Theoretical kv cache sizes are calculated only when cache_factory is None (we are skipping measurement as it will be incorrect due to additional s and z values)
        ...
        cache = cache_factory()
        with torch.inference_mode():
            output = model(input_ids, past_key_values=cache, use_cache=True, logits_to_keep=1)
    ...
    prefill_timings, prefill_median = timing.time_prefill(model, input_ids, cache_factory, n_runs, n_warmup)
